...
async def test_list_command(tmp_path):
    await seed_store(tmp_path, "run-cli1", 3)
...
async def test_list_empty_run(tmp_path):
    result = runner.invoke(app, ["list", "nonexistent-run", "--store", str(tmp_path)])
    assert result.exit_code == 0
    assert "No checkpoints" in result.output


async def test_inspect_command(tmp_path):
    await seed_store(tmp_path, "run-cli2", 3)
...
async def test_inspect_missing_step(tmp_path):
    await seed_store(tmp_path, "run-cli2b", 2)
    result = runner.invoke(
...
async def test_resume_command_prints_snippet(tmp_path):
    await seed_store(tmp_path, "run-cli3", 2)
    result = runner.invoke(app, ["resume", "run-cli3", "--store", str(tmp_path)])
    assert result.exit_code == 0
    assert "DurableAgentLoop" in result.output


async def test_resume_no_checkpoints(tmp_path):
    result = runner.invoke(app, ["resume", "nonexistent-run", "--store", str(tmp_path)])
    assert result.exit_code == 1
    assert "No checkpoints" in result.output


async def test_prune_command(tmp_path):
    await seed_store(tmp_path, "run-cli4", 5)
    result = runner.invoke(
        app, ["prune", "run-cli4", "--keep", "2", "--store", str(tmp_path)]
    )
    assert result.exit_code == 0
    store = DiskStore(base_dir=tmp_path)
    metas = await store.list("run-cli4")
    assert len(metas) == 2


async def test_delete_command(tmp_path):
    await seed_store(tmp_path, "run-cli5", 2)
    result = runner.invoke(app, ["delete", "run-cli5", "--store", str(tmp_path)])
    assert result.exit_code == 0
    store = DiskStore(base_dir=tmp_path)
    metas = await store.list("run-cli5")
    assert metas == []


async def test_stats_command(tmp_path):
    await seed_store(tmp_path, "run-cli6", 4)
    result = runner.invoke(app, ["stats", "--store", str(tmp_path)])
    assert result.exit_code == 0
    assert "total" in result.output.lower()


async def test_stats_empty_store(tmp_path):
...
async def test_stats_no_store(tmp_path):
